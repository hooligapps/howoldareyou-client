...
-   Библиотека создает новое окно с полученным URL.
-   Библиотека устанавливает обработчик событий `window.addEventListener('message', ...)` для получения результата из нового окна.

#### **Обработка событий**

-   Обработчик `message` должен строго проверять `event.origin` на соответствие `verificationApiDomain`.
-   Когда из `iframe` приходит сообщение о завершении (`{result: status, sessionId: sessionId, signature: signature}`), библиотека должна:
    ...
    3.  Закрыть новое окно (должно закрываться само) и снять обработчик событий `message` для очистки.
...
Модуль должен представлять собой набор чистых функций, не хранящих состояние. Вся необходимая информация (включая `apiKey` и `appId`) передается в качестве аргументов.
...
1.  **`start_check_age_verification(api_id: str, api_key: str, session_id: str, client_ip: str, redirect_url: str, user_id: str = None) -> dict`**
    ...
    -   Возвращает ответ от API в виде словаря (e.g. `{result: 0}` или `{"result": 1, url: "redirect-url"}`).
...
-   **Структура данных в колбэках:** Колбэки `onSuccess`, `onFail`, `onVerificationNeeded` и `onError` должны получать полную, необработанную структуру ответа (JSON), которую возвращает сервер Игры.
